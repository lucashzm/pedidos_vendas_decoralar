async function gerarPDF(idPedido){
 ...
 const campo=(label,valor)=>{
  ...
  const x=margem+doc.getTextWidth(label);
 ...
 doc.setFontSize(24);
 doc.text('Decoralar',margem,y);
 doc.setFont('helvetica','normal');
 doc.setFontSize(9);
 doc.setTextColor(105,105,105);
 doc.text('PEDIDO DE VENDA',direita,y-3,{align:'right'});
 doc.setFontSize(10);
 doc.setTextColor(45,45,45);
 doc.text(`Nº ${pedido.numero_pedido}`,direita,y+3,{align:'right'});
 y+=11;
 linha();
 ...
 doc.setFontSize(10);
 doc.setTextColor(35,35,35);
 doc.text('Observações de entrega:',margem,y);
 y+=5;
 doc.setFont('helvetica','normal');
 const obsEntrega=doc.splitTextToSize(pedido.observacoes||'',170);
 doc.text(obsEntrega,margem,y);
 y+=(obsEntrega.length*4)+5;
 doc.setFont('helvetica','bold');
 doc.text('Previsão de entrega: ',margem,y);
 const xPrev=margem+doc.getTextWidth('Previsão de entrega: ');
 doc.setFont('helvetica','normal');
 doc.text(formatarDataBR(pedido.previsao_entrega),xPrev,y);
 y+=10;
 ...
 itens?.forEach(i=>{
  ...
  doc.setFontSize(10);
  doc.setTextColor(35,35,35);
  const l=doc.splitTextToSize(i.produto,120);
  doc.text(l,margem,y);
  doc.text(String(i.quantidade),150,y);
  doc.text(formatarBRL(i.valor_unitario),direita,y,{align:'right'});
  y+=(l.length*5)+6;
 });

 linha();
 titulo('RESUMO FINANCEIRO');
 campo('Forma de pagamento: ',pedido.forma_pagamento||'');
 ...
 doc.text('Pedido de venda • Decoralar',105,y,{align:'center'});
