async function gerarPDF(idPedido){
 ...
 doc.setTextColor(30,30,30);doc.setFont('times','italic');doc.setFontSize(24);doc.text('Decoralar',margem,y);
 doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(105,105,105);doc.text('PEDIDO DE VENDA',direita,y-3,{align:'right'});doc.setFontSize(10);doc.setTextColor(45,45,45);doc.text(`Nº ${pedido.numero_pedido}`,direita,y+3,{align:'right'});y+=11;linha();
 ...
 y+=5;doc.setFont('helvetica','bold');doc.setFontSize(14);doc.setTextColor(25,25,25);doc.text('TOTAL DO PEDIDO',margem,y);doc.setFontSize(15);doc.text(formatarBRL(pedido.valor_total||0),margem+doc.getTextWidth('TOTAL DO PEDIDO')+8,y,{align:'left'});y+=19;
 doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(125,125,125);doc.text('Pedido de venda • Decoralar',105,y,{align:'center'});doc.save(`Pedido_Venda_${pedido.numero_pedido}.pdf`);
}
